...
```{r anova-sim-solution, exercise = TRUE}
null_dist <- gss %>% 
  specify(wordsum ~ class)  %>% 
  hypothesise(null = "independence") %>% 
  generate(reps = 1000, type = "permute") %>% 
...
t_test_results <- 
  pairwise.t.test(x = gss$___, g = gss$___, p.adjust.method = "___")
...
pairwise.t.test(gss$wordsum, gss$class, p.adjust.method = "none")
```
...
# Run a pairwise t-test on wordsum and class, without adjustment
t_test_results <- pairwise.t.test(x = gss$wordsum, g = gss$class, p.adjust.method = "bonferroni")
