...
```{r get_first_setup, message=FALSE, warning=FALSE}
get_first <- function(x) {
  x |>
    # Convert the numbers to strings
    as.character() |>
...
```{r ex5, exercise=TRUE, exercise.setup="get_first_setup"}
# Print get_first
get_first
...
<<<<<<< HEAD
```{r p_benford_setup, message=FALSE, warning=FALSE}
...
```{r ex6, exercise=TRUE, exercise.setup="p_benford_setup"}
# Inspect p_benford
p_benford
...
```{r ex7_setup, message=FALSE, warning=FALSE}
set.seed(123)
...
```{r ex7, exercise=TRUE, exercise.setup="ex7_setup"}
...
```{r ex8, exercise=TRUE, exercise.setup="ex7_setup"}
# From previous step
degrees_of_freedom <- iran |> 
  pull(first_digit) |> 
  nlevels() - 1
...
```{r ex12, exercise=TRUE, exercise.setup="iowa_setup"}
# Compute observed stat
chi_obs_stat <- ___ |>
  ___
```
...
```{r ex12-hint}
The two argument required of the `chisq_stat()` function are the `response` variable and vector of probabilities, `p`, which are already saved to your workspace as `p_benford.`
```
...
```{r ex13, exercise=TRUE, exercise.setup="iowa_setup"}
# Form null distribution
null <- ___
  # Specify response
  ___
...
```{r ex13-hint}
The data here is generated by `"simulate"`-ing from a ``"point"`` null hypothesis where `p = p_benford`.
```
...
```{r ex14, exercise=TRUE, exercise.setup="iowa_setup"}
# From previous steps
chi_obs_stat <- iowa_county |>
  chisq_stat(response = first_digit, p = p_benford)
null <- iowa_county |>
...
<<<<<<< HEAD
```{r iowa_setup2, message=FALSE, warning=FALSE}
p_benford <- 1:9 %>% setNames(log10(1 + 1 / .), .)
=======
```{r message=FALSE, warning=FALSE}
p_benford <- 1:9 |> (\(x) setNames(log10(1 + 1/x),x))()
