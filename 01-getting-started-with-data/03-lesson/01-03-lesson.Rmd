...
This data frame contains data from counties in all 50 states plus the District of Columbia, or DC, as it's commonly referred to. Since DC is not a state by definition, we'll first remove it from the dataset. We can do this with the `filter()` function that you learned about earlier in the tutorial.

That is, we will create a new data frame, called `county_noDC` that `filter()`s out the observation where state is DC. We'll also `droplevels()` for good measure, so that the level District of Columbia is removed completely from the data frame.
...
To do so we will start with the `county_srs` data frame: first group these data by state and then, count the number of observations per state.
...
The resulting output shows that this sampling approach yielded data from only _45_ states and the number of counties sampled varies from state to state.
...
quiz(
  question("Which of the below is correct?", correct = "Nice job!", allow_retry = TRUE,
    answer("There are 3 explanatory variables (light, noise, gender) and 1 response variable (exam performance).", message = "Close, but gender is thought to be a confounding variable, not an explanatory one!"),
    answer("There is 1 explanatory variable (gender) and 3 response variables (light, noise, exam performance).", message = "Not quite!"),
    answer("There are 2 blocking variables (light and noise), 1 explanatory variable (gender), and 1 response variable (exam performance).", message = "Incorrect!"),
    answer("There are 2 explanatory variables (light and noise), 1 blocking variable (gender), and 1 response variable (exam performance).", correct = TRUE)), caption = "")
```
