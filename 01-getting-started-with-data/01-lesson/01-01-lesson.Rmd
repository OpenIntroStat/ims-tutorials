...
female. It should be noted that the language of government surveys, such as High
School and Beyond, is slow to change. So with these types of data, you will
...
the race variable, which has levels of white, african american, hispanic, and
asian.
...
For the `email50` dataset, the `number` variable has three levels, "none", "some", 
and "big". We want to create a new variable that combines the levels "some" and
"big" into one level named "yes", and keeps the "none" level separate as a level 
named "no". In this exercise we will use the `case_when()` function, a cousin 
of the `if_else()` function. The `case_when()` function behaves similarly to the 
`if_else()` function, but where `if_else()` only accepts two options (what to 
do when the comparison is `TRUE` and `FALSE`), `case_when()` accepts as many 
options as you want to give it! 



- Create a new column in `email50` called `number_cat` that is `"no"` if there is no number in the email and `"yes"` otherwise. Use the `case_when()` function to accomplish this. 
- Assign the resulting data frame to a new data frame named `email50_updated`.
...
# Create number_cat column in email50
 email50_updated <- email50 %>%
   mutate(<NAME OF NEW VARIABLE> = 
            case_when(
              # if number is "none", make number_cat "no"
              <LOGICAL COMPARISON> ~ "<VALUE>",
              # if number is "small", make number_cat "yes"
              <LOGICAL COMPARISON> ~ "<VALUE>", 
              # if number is "big", make number_cat "yes"
              <LOGICAL COMPARISON> ~ "<VALUE>"
              )
          )
