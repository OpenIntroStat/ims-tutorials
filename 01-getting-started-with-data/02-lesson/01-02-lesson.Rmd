...
Next we'll calculate the proportion of males and proportion of females admitted, by creating a new variable, called `prop` (short for proportion) based off of the counts calculated in the previous exercise and using the `mutate()` function.
...
2. Create the a new variable, `prop`, as the proportion of males and females admitted. Remember, Proportions for each row of the data frame we create can be calculated as `n / sum(n)`! 
3. Filter the data for those who are admitted so that the resulting data frame only contains the proportions of males and females who are admitted.
...
First, assign `ucb_admission_counts` as the `count()` of `ucb_admit` by department, gender, and admission status.
...
1. Group `ucb_admission_counts` by department then gender.
2. Add a new column, `prop`, which is the ratio of those admitted or rejected by department and gender.
3. Filter the rows for those who are males **and** those who who were admitted.
