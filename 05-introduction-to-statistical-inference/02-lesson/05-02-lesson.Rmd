...
Do you want the proportion of women who were promoted or the proportion of promoted individuals who were women? Here, you want the first of these, so in your R code it's necessary to group by `gender` first.
...
- Using the permutation dataset, `gender_discrimination_perm` (which we already calculated), plot `stat`.
- Add a histogram layer with `geom_histogram()`. The `binwidth` is set to `0.01`.
- Add a vertical line with `geom_vline()`. The `xintercept` is at `diff_orig` (which we calculated earlier).
...
A p-value is the _probability of observing data as or more extreme than what we actually got, given that the null hypothesis is true_.

In this example, the p-value is the _probability of observing a difference of 0.2917 or greater when promotion rates do not vary across gender = 0.03_.
...
- Repeat for the small dataset, `gender_discrimination_small_perm`, which has observed difference `diff_orig_small`.
- Repeat for the big dataset, `gender_discrimination_big_perm`, which has observed difference `diff_orig_big`.
...
<div id="pvalue-hint">
**Hint:** Argument of the both functions should be `obs_stat = diff_orig, direction = "greater"`.
</div>
...
Well done! You may have noticed that the observed statistic is borderline significant (p-value = 0.03) for the original data. However, the same statistic would have been very consistent with the small dataset (p-value = 0.277 ) and extremely unusual for the large dataset (p-value = 0).
...
The `gender_discrimination_perm` and `gender_discrimination_perm` datasets are available in your workspace.

 (1)
- Run the code to recall the counts of `gender` and `decision` in the original dataset.
- Get the counts in the new dataset, `gender_discrimination_new`.
...
- Draw the histogram with the new permutation dataset, `gender_discrimination_new_perm`. On this plot, set the x-axis intercept of the vertical line to `diff_orig_new`.
...
- Calculate the p-value with the new permutation dataset, `gender_discrimination_new_perm`, and the new observed difference, `diff_orig_new`.
...
Because 0.03 is less than 0.05, we reject the null hypothesis and claim that men are promoted at a higher rate than women. That is, we conclude that it was not simply random variability which led to a higher proportion of men being promoted. A p-value of 0.03 is reasonably close to 0.05 which means we should be somewhat careful in making strong claims. We should take the results as a indication that more work should be done on the claims. Indeed, in 40 years since this research was published, many social scientists have been able to replicate research on gender discrimination in the workforce.
