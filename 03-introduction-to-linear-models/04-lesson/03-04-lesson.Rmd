---
title: "Introduction to Linear Models: 4 - Interpreting regression models"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
To get those values, we'll use the `lm()` command to actually fit the model. We specify two arguments to `lm()`: a `formula` that indicates which variable is the response and which is the explanatory; and a `data` argument that identifies the data frame where those variables are located. 

```{r 5}
lm(ucla_new ~ amaz_new, data = textbooks)
```

The output from `lm()` reminds us how we called it, and then gives us the fitted coefficients. The value of the intercept coefficient---$\hat{beta}_0$---is 0.929 dollars, while the value of the slope coefficient---$\hat{\beta}_1$---is one dollar and twenty cents. 

### Regression equation

$$
  \widehat{ucla_{new}} = 0.929 + 1.199 \cdot amaz_{new}
$$


What this says is that for each additional dollar that Amazon charges for a book, the UCLA bookstore charges about $1.20. In effect, the prices at the UCLA bookstore are about 20% higher than those on Amazon. 
...
question("Which of the following is the correct interpretation of the slope coefficient?",
  answer("Among U.S. counties, each additional percentage point increase in the poverty rate is associated with about a 0.591 percentage point decrease in the high school graduation rate. "),
  answer("Among U.S. counties, each additional percentage point increase in the high school graduation rate is associated with about a 0.591 percentage point decrease in the poverty rate.", correct = TRUE, message = "Remember that the slope represents the estimated change in the response variable for a one unit change in the explanatory variable."),
  answer("Among U.S. counties, each additional percentage point increase in the high school graduation rate is associated with about a 0.591 percentage point increase in the poverty rate."),
...
```{r ex1-bdims, exercise = TRUE}
# Linear model for weight as a function of height
lm(___ ~ ___, data = ___)
```

<div id="ex1-hint">
**Hint:** The formula `y ~ x` means `y` is a function of `x`.
</div>

```{r ex1-bdims-solution}
# Linear model for weight as a function of height
...
```{r ex1-bdims-check}
grade_result(
  pass_if(~identical(floor(.result$coefficients[1]),-106), "You have fitted the model correctly!"),
  fail_if(~identical(floor(.result$coefficients[1]), 136), "We want a linear model where weight is a function of height, not the other way round. "),
  fail_if(~TRUE, "Not quite, review the Slope & Intercept section where the lm function is introduced.")
)
```

```{r ex1-mlb, exercise = TRUE}
...
```{r ex1-mlb-hint-1}
Look at the previous exercise. They are pretty similar!

```

```{r ex1-mlb-hint-2}
# Linear model for SLG as a function of OBP 
lm(___ ~ ___, data = mlbbat10)
```
...
```{r ex1-mlb-check}
grade_result(
  pass_if(~identical(round(.result$coefficients[2],digits =2), 1.11), "You have fitted the correct model!"),
  fail_if(~identical(round(.result$coefficients[1],digits = 3), 0.064), "Make sure you are modelling slg as a function of obg, not the other way round."),
  fail_if(~TRUE, "Check out the previous exercise. They are pretty similar!")
)
```

```{r-ex1-mammals, exercise = TRUE}
# Log-linear model for body weight as a function of brain weight

```

```{r-ex1-mammals-hint-1}
# Log-linear model for body weight as a function of brain weight
lm(___ ~ ___, data = ___)
```

```{r-ex1-mammals-hint-2}
# Log-linear model for body weight as a function of brain weight
lm(log(___) ~ log(___), data = ___)
```

```{r ex1-mammals-solution}
# Log-linear model for body weight as a function of brain weight
...
```{r ex1-mammals-check}
grade_result(
  pass_if(~identical(round(.result$coefficients[1],digits = 1), -2.5), "You have fitted the correct model!"),
  fail_if(~identical(floor(.result$coefficients[1]), -57), "Did you forget to apply the log function to your variables? Remember, we want a log-linear model."),
  fail_if(~identical(round(.result$coefficients[1], digits =2), 2.13), "We want body weight as a function of brain weight, not the other way round."),
  fail_if(~TRUE, "Not qite, look at the hints for help.")
)
...
You can also return just the fitted coefficients as a vector using the `coef()` function. For our purposes, these are the pieces of information that we are most interested in. 
...
Since the object `mod` contains everything R knows about our model, we can ask R for the fitted values, using the `fitted.values()` function. This returns a vector containing the $\hat{y}$ values for each data point. 
...
<div id="ex1-hint">
**Hint:** Use `coef()` and `summary()` on the model object.
</div>
...
```{r ex4-hint-1}
mean(___) == mean(fitted.values(mod))
```
...
<div id="ex7-hint">
**Hint:** `hgt` variable in the `coefs` data frame indicates the slope.
</div>
...
```{r ex7-check}
grade_code("You have created the correct plot!")
```
