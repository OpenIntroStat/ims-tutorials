...
Since the object `mod` contains everything R knows about our model, we can ask R for the fitted values, using the `fitted.values()` function. This returns a vector containing the $\hat{y}$ values for each observation in our dataset.
...
`mod` (defined above) is available in your workspace. We will use the `all.equal()` function to confirm that the mean of the body weights equals the mean of the fitted values of `mod`. This function takes two inputs (separated by a comma) that you would like to check if they are equal (up to a tolerance of 1.5e-8). 
*Hint*: Remember, you can extract a specific column of a `data.frame` using the `$`. 

Next, compute the mean of the residuals of `mod` and check if it is equal to 0.
...
```{r ex4-hint-1}
all.equal(mean(bdims$wgt), mean(fitted.values(mod))) 
```
...
- Create a new data frame called `bdims_tidy` that is the `augment`ation of the `mod` linear model. 
...
<div id="ex7-hint">
**Hint:** The `hgt` variable in the `coefs` data frame indicates the slope.
</div>
