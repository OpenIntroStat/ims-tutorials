...
After counting the number of observations at each level of `id` and `align`, we use the `mutate()` function to calculate the proportion of observations that fell in those levels. We save these proportions into a new variable named `prop`. 
...
```{r cpkable, echo=FALSE,out.width = 10}
comics %>% 
  count(id, align) %>%
  group_by(id) %>% 
  mutate(prop = n / sum(n)) %>% 
