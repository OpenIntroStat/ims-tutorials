...
change <- change |>
  mutate(Small= Dimes + Nickels + Pennies) |>
  mutate(Amount = Dollars) |>
>>>>>>> 75d94152fd554e2ed0d65b25c99dfedaa014cc94
...
Consider data collected by Andrew Bray at Reed College on characteristics of LA Homes in 2010. T
...
```{r quiz_1}
quiz(
  question("What is to provide the appropriate interpretation of the coefficient on `log(sqft)`?  
", 
           allow_retry = TRUE,
    answer("Each additional square foot of house size produces an estimate of the average price which is $1.44 more.", message = "Remember that a log-log model has a special kind of interpretation."),
  answer("Each additional square foot of house size produces an estimate of the average price which is $1,442 more.", message = "Remember that a log-log model has a special kind of interpretation."),
  answer("Each additional square foot of house size produces an estimate of the average price which is 1.44% higher.", message = "Remember that a log-log model has a special kind of interpretation."),
  answer("Each additional 1% of square footage produces an estimate of the average price which is $1.44 more.", message = "Remember that a log-log model has a special kind of interpretation."),
    answer("Each additional 1% of square footage produces an estimate of the average price which is 1.44% higher.", correct = TRUE)
  ), caption = ""
)
```
...
quiz(
  question("What is the correct interpretation of the coefficient on `Service` in the linear model which regresses `Price` on `Service`, `Food`, and `Decor`?", 
           allow_retry = TRUE,
    answer("For every one unit increase in `Service`, the predicted average `Price` is expected to increase by 0.135, given fixed values of `Food` and `Decor`.", correct = TRUE),
    answer("For every one unit increase in `Service`, the predicted average `Price` is expected to increase by 0.135.", message = "You have to consider the interpretation of a single variable while holding the other variables constant."),
  answer("For every one unit increase in `Service`, the predicted average `Price` is expected to increase by 0.135, for any possible value of `Food` and `Decor`.", message = "You have to consider the interpretation of a single variable while holding the other variables constant."),
  answer("Given that `Food` and `Decor` are in the model, `Service` is not significant, and we cannot know whether it has effect on modeling `Price`.", message = "You have to consider the interpretation of a single variable while holding the other variables constant.")
  ), caption = ""
)
```
