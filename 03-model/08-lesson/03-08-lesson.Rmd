...
The following example uses the [`babies` data](https://www.openintro.org/data/index.php?data=babies) set from the `openintro` package together with our ‘tidy’-toolkit collected in the tidyverse package. We are going to model the birthweight (`bwt`) of babies born in San Francisco between 1960 and 1967 as a function of their pregnancy’s length of `gestation` (in weeks) and the mother’s `age` (in years).
...
1. Again we start with the generation of an evenly spaced grid of points from the data. But instead to use every possible combination of both variables (`by = 1`) we build a symmetrical plane of 50 points (`n = 50`).

2. The second step is the same: We feed those generated grid values into our model.

3. We build the regression plane as a matrix of the unique values of both variables as we had done previously in the sandbox and extract (`pull`) the vector of the `.fitted` value from our model.

4. Instead of `ggplot2` we use the `plotly` package for the visualization. The syntax of the `plot_ly` function is very similar to that of `ggplot.` In this case, the `add_markers()` function draws the points, while the `add_surface()` function draws the plane. 
...
Move the model around with your mouse. Visualizing the structure from different angles helps a lot to see what is going on.
...
```{r babies-model-space, echo=TRUE, exercise=TRUE}
model_space
```
...
```{r babies-model-hline, echo=TRUE, exercise=TRUE}
model_space + 
...
```{r babies-model-vline, echo=TRUE, exercise=TRUE}
model_space + 
...
```{r babies-plotly-two, exercise.setup="ex3-babies-tiling-plane", exercise=TRUE, echo=TRUE}
grid <- babies %>%
  modelr::data_grid(
    gestation = seq_range(gestation, n = 50),
    age = seq_range(age, n = 50), 
